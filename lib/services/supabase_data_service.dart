...
class SupabaseDataService {
  ...
  Future<List<Workout>> getWorkouts() async {
    final rows = await _db.from('workouts').select('*, exercises(*, set_entries(*))').eq('user_id', userId).order('name');
  ...
  Future<void> updateWorkout(Workout w) => _db.from('workouts').update({'name':w.name,'scheduled_weekdays':w.scheduledWeekdays,'scheduled_hour':w.scheduledHour,'scheduled_minute':w.scheduledMinute}).eq('id',w.id!);
